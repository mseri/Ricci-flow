...
void draw_full_surface() // draw the surface
{
  ...
  for (j=skip; j<N-2-skip; j+=skip) // y direction first
  {
  ...
  glBegin(GL_TRIANGLE_FAN); // the top and bottom round part
  ...
  for (j=0; j<16; j++) //horizontal lines 
  {
    glBegin(GL_LINE_STRIP);
    if (choose_surface==1) glColor3f((GLfloat)0.0,(GLfloat)0.0,(GLfloat)1.0);
    else if (still_ok==1) glColor3f((GLfloat)0.0,(GLfloat)0.2,(GLfloat)0.0);
...
void draw_surface()
{
  ...
  if (metric==1) // draw the metric and the line frame
  {
    glDisable(GL_LIGHTING);
    glBegin(GL_POINTS);
...
void myReshape(int w, int h)
{
  ...
  float matrix[16];
  glhPerspectivef2(matrix, 40.0,(GLfloat)w/(GLfloat)h,0.001,1000.0);
  //gluPerspective(40.0,(GLfloat)w/(GLfloat)h,1.0,100.0);
  // fovy in degrees, x/y, znear, zfar
